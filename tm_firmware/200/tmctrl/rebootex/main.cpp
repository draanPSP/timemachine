...
namespace {
	...
	inline auto const cache1Ptr = reinterpret_cast<v_v_function_t const>(0x88c040d8);
	inline auto const cache2Ptr = reinterpret_cast<v_v_function_t const>(0x88c03bfc);
...
int main(s32 const a0, s32 const a1, s32 const a2, s32 const a3) {
	...
	MAKE_CALL(0x88c010ec, sysMemModuleStartPatched);
	_sw(0x02403021, 0x88c010f0);
	
	_sw(0x02203021, 0x88c010c8);
	MAKE_JUMP(0x88c010d0, loadCoreModuleStartPatched);

	// Patch removeByDebugSection, make it return 1	
	_sw(0x03e00008, 0x88c01de4);
	_sw(0x24020001, 0x88c01de8);

	// Disable sign check descramble of boot configs, sysmem and loadcore
	_sw(0x3C040000, 0x88c004cc);

	// disable setting of sign checked module attr flag on load core module info
	_sw(0, 0x88c00934);

	/* File open redirection */
	MAKE_CALL(0x88c0007c, sceBootLfatMountPatched);
