...
void PatchSceLoadExec()
{
	...
	if (mod != NULL)
	{
		MAKE_CALL(mod->text_addr + 0x24cc, RebootBinDecompressPatched);
		_sw(0x3C0188FC, mod->text_addr + 0x2504); //LUI $at 0x88FC - Patch call to reboot.bin
...
void PatchSceModuleManager()
{
	...
	if (mod != NULL)
	{
		MAKE_JUMP(mod->text_addr + 0x5b10, sceKernelCreateThreadPatched);
		MAKE_JUMP(mod->text_addr + 0x5b20, sceKernelStartThreadPatched);
...
int OnModuleStart(SceModule2 *mod)
{
	...
	{
		REDIRECT_FUNCTION(text_addr + 0x5684, ValidateSeekPatched);
		REDIRECT_FUNCTION(text_addr + 0x5708, ValidateSeekP1Patched);
...
void PatchLoadCore()
{
	...
	if (mod != NULL) {
		...
		_sw(_lw(mod->text_addr + 0x26F8), mod->text_addr + 0x31a0);
		_sw(_lw(mod->text_addr + 0x26FC), mod->text_addr + 0x31a4);

		KERNEL_HIJACK_FUNCTION(mod->text_addr + 0x31a0, sceKernelCheckExecFileHook, _sceKernelCheckExecFile);
	}
}
