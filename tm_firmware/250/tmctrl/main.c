...
void PatchSceLoadExec()
{
	...
	if (mod != NULL)
	{
		MAKE_CALL(mod->text_addr + 0x2668, RebootBinDecompressPatched);
		_sw(0x3C0188FC, mod->text_addr + 0x26A4); //LUI $at 0x88FC - Patch call to reboot.bin
...
void PatchSceModuleManager()
{
	...
	if (mod != NULL)
	{
		MAKE_JUMP(mod->text_addr + 0x5cec, sceKernelCreateThreadPatched);
		MAKE_JUMP(mod->text_addr + 0x5cfc, sceKernelStartThreadPatched);
		ClearCaches();
	}
}
...
int OnModuleStart(SceModule2 *mod)
{
	...
	{
		_sw((u32)sceRegGetKeyInfoPatched, text_addr+0x6e84);
		_sw((u32)sceRegGetKeyValuePatched, text_addr+0x6e88);
	...
	{
		REDIRECT_FUNCTION(text_addr + 0x9a8, ValidateSeekPatched);
		REDIRECT_FUNCTION(text_addr + 0xa30, ValidateSeekP1Patched);
		GetMsSize = (void *)(text_addr + 0x0344c);
...
void PatchLoadCore()
{
	...
	if (mod != NULL) {
		...
		_sw(_lw(mod->text_addr + 0x2890), mod->text_addr + 0x352c);
		_sw(_lw(mod->text_addr + 0x2894), mod->text_addr + 0x3530);

		KERNEL_HIJACK_FUNCTION(mod->text_addr + 0x352c, sceKernelCheckExecFileHook, _sceKernelCheckExecFile);
