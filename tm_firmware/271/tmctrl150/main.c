...
int RebootBinDecompressPatched(u8 *dest, int destSize, u8 *src, int unk)
{
	...
	memcpy((void *)0x88fc0000, oerebootex + 0x10, 0x7fc);

	sceKernelGzipDecompress((u8 *)0x88fd0000, 0x10000, rebootex_01g, 0);

	if (*isReboot)
		return sceKernelDeflateDecompress(dest, destSize, rebootbin271 + 0x10, unk);
...
int OnModuleStart(SceModule2 *mod)
{
	...
	{
		_sw(0x344288fd, text_addr + 0x138); //LUI $v0, 0x88fd - Patch call to oe rebootex.bin

		REDIRECT_FUNCTION(text_addr + 0x1c, RebootBinDecompressPatched);

		rebootbin271 = (u8 *)(text_addr + 0x608);
		oerebootex = (u8 *)(text_addr + 0xce34);
		isReboot = (int *)(text_addr + 0xd640);
