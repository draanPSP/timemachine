...
namespace {
	...
	inline auto const cache1Ptr = reinterpret_cast<v_v_function_t const>(0x8841489c);
	inline auto const cache2Ptr = reinterpret_cast<v_v_function_t const>(0x884143c0);
...
int main(s32 const a0, s32 const a1, s32 const a2, s32 const a3) {
	...
	MAKE_CALL(0x88401294, sysMemModuleStartPatched);
	_sw(0x02403021, 0x88401298);
	
	// mov $a0, $a3 -> mov $a2, $s4 (a0 = 8 -> a2 = module_start)
	// mov $a1, $sp
	// jr  loadcore::module_start -> j LoadCoreStart 
	// mov $sp, $v1
	_sw(0x02803021, 0x88401248);
	MAKE_JUMP(0x88401250, loadCoreModuleStartPatched);

	/* NoPlainPspConfigCheckPatch */

	_sw(0xafa50000, 0x8840d4f4);
	_sw(0x20a20000, 0x8840d4f8);

	// Patch removeByDebugSection, make it return 0
	MAKE_FUNCTION_RETURN0(0x884135cc);

	// Disable sign check descramble of psp boot configs, sysmem and loadcore
	_sw(0x3c040000, 0x88400f20);
	...
	MAKE_CALL(0x88402168, sceBootLfatMountPatched);
	MAKE_CALL(0x88402178, sceBootLfatOpenPatched);
	MAKE_CALL(0x884021a4, sceBootLfatReadPatched);
	MAKE_CALL(0x884021c4, sceBootLfatClosePatched);
